...
CLI alias: ``python -m kaye a p`` — verifies all skill SKILL.md files and
the plugin.json manifest are exported.
"""
...
def testee_plugin_folder(tmp_path_factory, cli_command):
    ...
    root = prepare_root_folder(tmp_path_factory, command, "plugin_a_p")
    return root / "kaye"
