"""
cli-a-p-alts-claude-p_test.py

Unit Tests (using pytest) for:

CLI alias: ``python -m kaye claude p`` — verifies all skill SKILL.md files
...
def testee_plugin_folder(tmp_path_factory, cli_claude_command):
    command = cli_claude_command + "p "
    root = prepare_root_folder(tmp_path_factory, command, "plugin_claude_p")
    return root / "kaye"
