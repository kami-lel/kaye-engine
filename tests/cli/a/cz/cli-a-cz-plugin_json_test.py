"""
cli-a-cz-plugin_json_test.py

Unit Tests (using pytest) for:

``python -m kaye claude plugin -z`` — verifies the plugin.json manifest is
exported under ``.claude-plugin/`` with the expected content within the
extracted .zip.
"""
