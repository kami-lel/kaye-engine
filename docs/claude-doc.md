...
| sidecar name | tool | Chat | Code | remark |
| --- | --- | --- | --- | --- |
...
| `{Claude Code [Agent] Usage}` | `Agent` | ❌ | ✔️ | launches a subagent for multi-step or research tasks |
| `{Claude Code [ListAgents] Usage}` | `ListAgents` | ❌ | ✔️ | lists other agents/sessions reachable via `SendMessage` |
| `{Claude Code [AskUserQuestion] Usage}` | `AskUserQuestion` | ❌ | ✔️ | asks the user a clarifying question with selectable options |
...
| `{Claude Code [Skill] Usage}` | `Skill` | ❌ | ✔️ | invokes a packaged skill (`/skill-name`) |
| `{Claude Code [ToolSearch] Usage}` | `ToolSearch` | ❌ | ✔️ | fetches full schemas for deferred tools by name or keyword |
| `{Claude Code [Write] Usage}` | `Write` | ❌ | ✔️ | creates or overwrites a file on local disk |
...
| `{Claude Code [TaskStop] Usage}` | `TaskStop` | ❌ | ✔️ | stops a background task |
