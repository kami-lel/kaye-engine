...
<!-- FIXME change corpus -> caller project requirement -->

## Corpus Requirements

A corpus must supply a node at `Agent Behavior` → `Claude Behavior`, and register a Chat blueprint and a Coder blueprint under whatever names the consumer passes to `set_claude_using_blueprint(chat_bp_name, coder_bp_name)`; `user_prompt/export.py` resolves them via `get_claude_chat_blueprint()`/`get_claude_coder_blueprint()` in `blueprint_name.py`.
