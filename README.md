...
The `(Abbreviations)` dynamic node reads its meanings from an `abbrs.json` file loaded via `load_abbr_data`/`get_abbr_data` — kaye-engine bundles no copy of its own; a host package (e.g. `kaye-vault`) supplies and loads the real file. Q.v. [`abbrs.json` documentation](docs/abbrs_json_doc.md) for its format.
